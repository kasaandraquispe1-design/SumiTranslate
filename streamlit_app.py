...
st.set_page_config(page_title="Sumire Translate", page_icon="🌸", layout="wide", initial_sidebar_state="collapsed")
...
.hero { padding:42px 0 26px; } .eyebrow { color:var(--sumire-primary); font-size:12px; letter-spacing:2px; font-weight:700; margin-bottom:10px; }
...
.main-title span { color:var(--sumire-primary-soft); font-style:italic; } .description { max-width:720px; margin-top:15px; font-family:Georgia,serif; font-size:18px; color:#64616c; line-height:1.55; }
.feature-row { display:flex; flex-wrap:wrap; gap:8px; margin-top:18px; } .feature-badge { display:inline-flex; align-items:center; gap:6px; padding:6px 10px; border:1px solid var(--sumire-border); border-radius:999px; background:#fff; color:#5e5b67; font-size:12px; }
.section-title { font-size:14px; font-weight:700; color:var(--sumire-ink); margin-bottom:4px; } .section-caption { color:var(--sumire-muted); font-size:12px; margin-bottom:12px; }
...
.protection-title { font-weight:700; color:var(--sumire-ink); margin-bottom:4px; } .protection-text { color:var(--sumire-muted); font-size:13px; line-height:1.5; }
...
.sumire-stats { display:flex; flex-wrap:wrap; gap:8px; margin:12px 0 2px; } .sumire-stat { padding:8px 12px; border:1px solid var(--sumire-border); border-radius:12px; background:#fff; color:#514d5d; font-size:12px; } .sumire-stat strong { color:var(--sumire-primary); }
...
with col1: source_name = st.selectbox("Idioma de origen", language_names, key="source_language_widget", format_func=lambda name: f"{language_flags.get(name, '')} {name}".strip())
with col2: st.button("⇄", use_container_width=True, help="Intercambiar idiomas", on_click=swap_languages)
with col3: target_name = st.selectbox("Idioma de destino", language_names, key="target_language_widget", format_func=lambda name: f"{language_flags.get(name, '')} {name}".strip())
...
with col2: clear_clicked = st.button("Limpiar", use_container_width=True, on_click=clear_app)
...
if translate_clicked:
    ...
    if not has_input: st.warning("Escribe un texto o sube un documento primero.")
    elif source_name == target_name: st.warning("El idioma de origen y el idioma de destino deben ser diferentes.")
    else:
        with st.status("Procesando Sumire Translate...", expanded=True) as status:
            ...
            try:
                        ...
                        try:
                            if suffix in {".png", ".jpg", ".jpeg", ".webp"}:
                                with st.status("Leyendo el texto de la imagen...", expanded=False) as image_status:
                                    source_text = extract_text_from_image(temp_path).strip(); image_status.update(label="Texto de la imagen recuperado", state="complete")
                            else: source_text = extract_text(temp_path).strip()
                        finally:
                            try: os.unlink(temp_path)
                            except OSError: pass
                            temp_path = None
                    if not source_text: st.warning("El documento no contiene texto reconocible.")
                    else:
                ...
                status.update(label="No se pudo completar la traducción", state="error"); st.error(str(exc))
            finally:
                if temp_path:
                    try: os.unlink(temp_path)
                    except OSError: pass
...
if st.session_state.table_detection:
    detected_tables = st.session_state.table_detection
...
if st.session_state.validation is not None:
    ...
    if validation.get("passed"): st.success("✓ Validación estructural superada: los elementos protegidos fueron restaurados exactamente.")
    else:
